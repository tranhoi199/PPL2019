...
class StaticChecker(BaseVisitor,Utils):
    ...
    def visitFuncDecl(self,ast, c):
        ...
        self.visit(ast.body,(c[0] + c[1], ast, param, sym, flag_func))
        return sym
    ...
    def visitBlock(self, ast, c):
        ...
        body = list(map(lambda x: self.visit(x, (lstSymofVar + c[0], ast, c[3] if type(x) is not Block else [], flag, False, c[5] if len(c) == 6 else None )), lstStmt))
        if c[4]:
    ...
    def visitReturn(self, ast , c):
        ...
        if not isinstance(c[2], list): # chi khi return trong function moi thuc hien viec tinh toan return
            funcType = c[2].mtype.rettype
            if ast.expr and type(c[2].mtype.rettype) is VoidType:
                raise TypeMismatchInStatement(ast)
            elif type(funcType) is not VoidType:
                if ast.expr:
                    #visitId using c[0] nen truyen vao (c[0], False)
                    expType = self.visit(ast.expr, (c[0],False))
                    # if type(c[2].mtype.rettype) is ArrayPointerType and type(expType) in [ArrayPointerType, ArrayType]:
                    #     if self.checkReturnFunctionCoersion(type(expType), funcType, expType.eleType, funcType.eleType):
                    #         return funcType
                    #     else:
                    #         raise TypeMismatchInStatement(ast)
                    # else:
    ...
    def visitDowhile(self, ast, c):
        ...
        if type(expType) is not BoolType:
            ...
            [self.visit(x,(c[0], ast, [], [True, True], flag_return, flag_break_continue)) for x in ast.sl]
    def visitFor(self, ast, c):
        ...
        if type(exp1Type) is IntType and type(exp3Type) is IntType and type(exp2Type) is BoolType:
            self.visit(ast.loop, (c[0], ast, [], [True, True], flag_return, flag_break_continue))
        else:
            raise TypeMismatchInStatement(ast)
