...
class StaticChecker(BaseVisitor,Utils):
    ...
    def visitFuncDecl(self,ast, c):
        ...
        # c = (x,global_evi)
        self.visit(ast.body,(c, ast, param, sym))
        return sym
    ...
    def visitBlock(self, ast, c):
        ...
        c[0][0].extend(lstSymofVar)
        # c[0] = (x,global_evi)
        body = list(map(lambda x: self.visit(x, (c[0], ast, c[3], flag)), lstStmt))
    ...
    def visitCallExpr(self, ast, c):
        ...
        res = self.lookup(ast.method.name,c[0][0],lambda x: x.name)
    ...
    def visitId(self, ast, c):
        res = self.lookup(ast.name,list(filter(lambda x: type(x.mtype) is not MType, c[0][0])) , lambda x: x.name)
        if res is None:
